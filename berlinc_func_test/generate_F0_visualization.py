...
def generate_F0_visualization(scriptfile):
    # Bands to test:
...
#    sys.path += ['/home/berlinc/anaconda3/envs/es/lib/python3.7/site-packages']
    print('[generate_F0_visualization] Instantiating EXOSIMS object.')
    sim = EXOSIMS.MissionSim.MissionSim(scriptfile)
    
    targetListNames = sim.TargetList.Name.copy()
    targetListSpecs = sim.TargetList.Spec.copy()
    
    print('[generate_F0_visualization] Checking TargetList.Spec against SYMBAD.')
    SB_in = []
    ...
    print('[generate_F0_visualization] Calling TargetList.F0() to compute stellar zero flux.')
    band, F0, atlas = [], [], []
    for LAM in bandeff:
        for stInd, spec in enumerate(targetListSpecs):
            ...
            if band_dict[LAM] in exceeding_bands:
                atlas.append('CK')
            else:
                atlas.append('Pickles')
    
    df_in = {'Band': band, 'F0 (ph/m2/nm/s)': F0, 'Atlas': atlas}
    df = pd.DataFrame(df_in)
    print(df)

    print('[generate_F0_visualization] Generating lineplot of F0 data.')
    plt.figure(figsize=(15, 7))
    ax = sns.lineplot(x="Band", y="F0 (ph/m2/nm/s)", data=df, hue="Atlas", ci="sd", 
                  estimator="mean", sort=False) 
    plt.title('F0 calc of target list - mean with 1 sigma moving range')
    filename = 'F0_calc_lineplot.png'
    plt.savefig(filename, dpi=300)
    print('[generate_F0_visualization] Lineplot saved as ' + filename + '.')

if __name__ == '__main__':
    scriptfile = sys.argv[1]
    generate_F0_visualization(scriptfile)

# Example usage:
# python generate_F0_visualization.py EXOSIMS/Scripts/HabEx_4m_coroOnly_DulzE_promo_EPRVmwants_lucky_charAsnr5_binMenn_20190929_mod.json
