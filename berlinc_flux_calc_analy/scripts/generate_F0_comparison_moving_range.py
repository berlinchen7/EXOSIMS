...
def generate_F0_comparison_moving_range(scriptfile, useSYMBAD=True):
    # Bands to test:
...
#    sys.path += ['/home/berlinc/anaconda3/envs/es/lib/python3.7/site-packages']
    ...
    band, F0, atlas = [], [], []
    for LAM in bandeff[:-2]: # Excluding L and M bands since they exceed 2.5 microns.
        for stInd, spec in enumerate(targetListSpecs):
            f0_p, mag = sim.TargetList.F0(band_BW_dict[LAM], LAM, stInd, spec, usePickles=True)
            f0_c, mag = sim.TargetList.F0(band_BW_dict[LAM], LAM, stInd, spec, usePickles=False)

            F0.append(f0_p.value)
            band.append(band_dict[LAM])
            atlas.append('Pickles')

            F0.append(f0_c.value)
            band.append(band_dict[LAM])
            atlas.append('CK')
    ...
    filename = '../plots/F0_calc_comparison_lineplot'
    if useSYMBAD:
...
if __name__ == '__main__':
    ...
    generate_F0_comparison_moving_range(scriptfile)

# Example usage:
# python generate_F0_comparison_moving_range.py EXOSIMS/Scripts/HabEx_4m_coroOnly_DulzE_promo_EPRVmwants_lucky_charAsnr5_binMenn_20190929_mod.json
